...
private:
    ...
    int GetThemeChangeReady(Creature* creature)
    {
        QueryResult result;
        result = WorldDatabase.PQuery("SELECT `time`, `refresh` FROM `gurubashi_givers` WHERE entry = %u", creature->GetEntry());
...
public:
    ...
    {
        char msg[100];
        int iTimeLast = GetThemeChangeReady(creature);

        if (iTimeLast <= time(0))
        {
            QueryResult result;
            result = WorldDatabase.PQuery("SELECT `id`, `name` FROM `gurubashi_themes` WHERE giver = %u", creature->GetEntry());
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
    {
        ...
        {
            if (GetThemeChangeReady(creature) <= time(0))
            {
                int iTimeCur = time(0);
                QueryResult result;

                WorldDatabase.PExecute("UPDATE `gurubashi_givers` SET `time` = %u WHERE `entry` = %u", iTimeCur, creature->GetEntry());
                ...
                if (result)
                {
                    creature->MonsterSay("Одолжите ваш волосик ... Трах-тебе-дох- быц-быц... фух готово...", LANG_UNIVERSAL, player->GetGUID());
                    do
                    {
                        Field* fields = result->Fetch();
                ...
                // Creatures
                result = WorldDatabase.PQuery("SELECT `x`, `y`, `z`, `o`, `entry`, `spawndist` FROM `gurubashi_spawnscreature` WHERE `theme` = %u", action - OFFSET_THEME);
                CRL::iterator i;
                ...
                if (result)
                {
                    ...
                    {
                        Field* fields = result->Fetch();
                        Creature* tempCr = creature->SummonCreature(fields[4].GetInt32(), fields[0].GetFloat(), fields[1].GetFloat(), fields[2].GetFloat(), fields[3].GetFloat(), TEMPSUMMON_MANUAL_DESPAWN, 0);
                        // Anti-hack
            ...
            else
            {
                ...
                creature->MonsterSay("Тема бала успешно изменена. Пожалуйста, подождите...", LANG_UNIVERSAL, player->GetGUID());
            }
        }
        else
        {
