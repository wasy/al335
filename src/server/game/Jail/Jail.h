...
{
    uint32 MaxJails;    // Beim erreichen dieses Wertes, wird der Charakter geloscht / der Account gebannt.
    uint32 MaxDauer;    // Die maximale Dauer fur einen Knastausenthalt.
    uint32 MinGrund;    // Minimale Zeichenlange fur den Jailgrund.
    uint32 MapAlly;     // Knastkarte fur die Allies
    uint32 MapHorde;    // Knastkarte fur die Horde
    ...
    uint32 Amnestie;    // Monate (nach dem letzten Vorfall), nach denen die Jaileintrage geloscht werden.

    Position AllyPos;   // Koordinaten fur den Allyknast
    Position HordePos;  // Koordinaten fur den Hordeknast

    bool WarnUser;      // Spieler warnen, wenn er nur einen Jail von der Charakterloschung / Accountbannung entfernt ist?
    bool DelChar;       // Charakter loschen, wenn m_MaxJails erreicht wird?
...
struct JailEintragStruktur
{
    ...
    std::string CharName;   // Charaktername des Haftlings
...
public:
    ...
    // Knast auf nicht mehr existierende Charaktere prufen.
